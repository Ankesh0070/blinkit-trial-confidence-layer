/* ============================================================
   Phase 2 — Pipeline-Driven Signals & Category Expansion
   Application Logic
   ============================================================ */
...
async function init() {
    try {
        // In a real app, this would be a fetch from the backend API
        // For the prototype, we fetch the static JSON files we generated
...
function renderCategoryNav() {
    ...
    // Only show Phase 2 target categories in the nav
...
function renderProducts() {
    ...
    products.forEach(product => {
        ...
        // Show badge if A/B toggle is ON and (QA is verified OR we're allowing preview)
        if (showBadges) {
            const trust = product.trust_signals;
            trustBadgeHTML = `
                <div class="trust-badge" onclick="event.stopPropagation(); openModal(${JSON.stringify(product).replace(/"/g, '&quot;')})">
...
function simulatePurchase() {
    ...
    // Simulate Phase 2 metrics logic
    // We assume 25% of trials lead to a repeat purchase (target > 15%)
    // We assume 5% lead to abandonment (spike > 10% is bad)
    if (Math.random() > 0.75) metrics.repeatPurchases++;
    if (Math.random() > 0.95) metrics.abandonments++;
    
    updateMetricsUI();
}

function updateMetricsUI() {
    const conversion = (metrics.trials > 0) ? "4.2%" : "0%"; // Static sim for conversion rate
    const repeatRate = (metrics.trials > 0) ? ((metrics.repeatPurchases / metrics.trials) * 100).toFixed(1) + "%" : "0%";
    const abandonRate = (metrics.trials > 0) ? ((metrics.abandonments / metrics.trials) * 100).toFixed(1) + "%" : "0%";
    ...
    document.getElementById('metricAbandon').textContent = abandonRate;
    
    const abandonCard = document.getElementById('metricAbandon').parentElement;
    if ((metrics.abandonments / metrics.trials) > 0.1) {
        abandonCard.style.borderLeftColor = 'var(--accent-red)';
        abandonCard.querySelector('.metric-note').textContent = 'CRITICAL: Spike detected';
        abandonCard.querySelector('.metric-note').style.color = 'var(--accent-red)';
    }
}
