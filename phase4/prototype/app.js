...
const THEMES = {
    ...
    'baby': { color: '#ff9f43', rgb: '255, 159, 67' },
    'home_cleaning': { color: '#36c2ce', rgb: '54, 194, 206' },
    'pet': { color: '#a67c52', rgb: '166, 124, 82' },
    'intimate_personal': { color: '#e84c88', rgb: '232, 76, 136' }
};
...
function evaluateConfidenceGate(categoryId) {
    ...
    // Check 1: Data Density (Phase 5 Full Override)
    // All sparse categories are now overridden for testing.
...
function renderRecommendationStrip() {
    ...
    const emojis = { 'electronics': '🔌', 'personal_care_beauty': '💄', 'pharmacy_health': '💊', 'baby': '🧸', 'home_cleaning': '🧹', 'pet': '🐕', 'intimate_personal': '🛡️' };
...
function renderCategoryNav() {
    ...
    categories.forEach(catId => {
        ...
        // Add a visual indicator if this is our sparse override test
        const isOverride = (flag === 'sparse' && ['baby', 'home_cleaning', 'pet', 'intimate_personal'].includes(catId));
        const densityClass = isOverride ? 'sparse-override' : flag;

        pill.innerHTML = `
            <span>${emojis[catId]}</span>
            <span>${catData.display_name}</span>
            <div class="density-dot ${densityClass}" title="${flag === 'dense' ? 'Dense Data' : 'Sparse Override'}"></div>
        `;
...
function selectCategory(categoryId) {
    ...
    const isOverride = ['baby', 'home_cleaning', 'pet', 'intimate_personal'].includes(categoryId);
    
    if (densityFlags.categories[categoryId].density_flag === 'sparse' && !isOverride) {
